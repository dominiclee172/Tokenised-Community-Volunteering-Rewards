...
(define-map volunteer-achievements
  { volunteer: principal, achievement-type: (string-ascii 50) }
  {
    earned-block: uint,
    season: uint,
...
(define-map seasonal-stats
  { volunteer: principal, season: uint }
  {
...
(define-read-only (get-volunteer-rank (volunteer principal))
  (let ((leaderboard-size-val (var-get leaderboard-size)))
    (fold check-volunteer-rank 
      (list u1 u2 u3 u4 u5 u6 u7 u8 u9 u10) 
      { target: volunteer, found-rank: none, current-rank: u1 })
  )
)

(define-read-only (get-volunteer-achievement (volunteer principal) (achievement-type (string-ascii 50)))
  (map-get? volunteer-achievements { volunteer: volunteer, achievement-type: achievement-type })
)
...
(define-read-only (get-seasonal-stats (volunteer principal) (season uint))
  (map-get? seasonal-stats { volunteer: volunteer, season: season })
)
...
(define-private (check-volunteer-rank (rank uint) (state { target: principal, found-rank: (optional uint), current-rank: uint }))
          ...
          reward-amount
            (begin
              ...
              (let (
                ...
                (try! (update-leaderboard-position tx-sender new-score))
                (try! (check-and-award-achievements tx-sender))
              )
...
(define-public (check-and-award-achievements (volunteer principal))
  (let (
    ...
    (if (and (is-eq (get total-activities stats) u1) 
             (is-none (get-volunteer-achievement volunteer "first-steps")))
      (map-set volunteer-achievements
        { volunteer: volunteer, achievement-type: "first-steps" }
        {
    ...
    (if (and (>= (get total-activities stats) u10) 
             (is-none (get-volunteer-achievement volunteer "dedicated-helper")))
      (map-set volunteer-achievements
        { volunteer: volunteer, achievement-type: "dedicated-helper" }
        {
          earned-block: stacks-block-height,
          season: current-season,
          milestone-value: u10,
    ...
    (if (and (>= (get total-hours stats) u100) 
             (is-none (get-volunteer-achievement volunteer "time-champion")))
      (map-set volunteer-achievements
        { volunteer: volunteer, achievement-type: "time-champion" }
        {
          earned-block: stacks-block-height,
          season: current-season,
